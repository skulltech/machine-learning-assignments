...
def neural_network(args):
    ...
    ylabelstest = ylabels[-test_samples:, :]
    ...
    while True:
        ...
        if i >= batches:
            epoch = epoch + 1
            probs, preds = nn.predict(xtest)
            time_elapsed = time.time() - start
            print(f'Epoch: {epoch}\t Accuracy: {accuracy(ylabelstest, preds)}\t Loss: {cross_entropy(ytest, probs)}\t Time: {time_elapsed}')
    ...
    probs, preds = nn.predict(x)
    print(f'Final. Accuracy: {accuracy(ylabels, preds)}\t Loss: {cross_entropy(y, probs)}')
